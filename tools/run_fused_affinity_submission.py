# -*- coding: utf-8 -*-
"""Submission inference from one fused semantic-affinity checkpoint."""
...
def main():
    ...
    parser.add_argument("--output-dir", default="outputs/submission_e10a_g4b_fused")
    ...
    model, bundle = load_fused_deployment_model(checkpoint_path, config, device)
    test_dir = Path(project_path(config, args.test_dir or infer_cfg["test_dir"]))
    ...
    for image_name in list_images(str(test_dir)):
        ...
        _, instance_map, class_map = postprocess(
            ...
            image_path.stem,
            infer_cfg,
            float(infer_cfg["boundary_threshold"]),
            bool(config.get("post_process", {}).get("save_visualization", True)),
            image_rgb=image,
    ...
    manifest = {
        ...
        "format": "fused_affinity_submission_v1",
        "checkpoint": str(checkpoint_path.resolve()),
        "checkpoint_sha256": file_sha256(checkpoint_path),
        "fused_format": bundle["format"],
        "sources": bundle["sources"],
        "parameter_summary": bundle["parameter_summary"],
        "semantic_source": "e10a_only",

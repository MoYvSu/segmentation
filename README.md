...
> 2026-09-25：[D5a语义完成与亮度诊断](docs/SEMANTIC_D5A_ANALYSIS_20260925.md)：A/B均完成60轮／3840更新，零失败、冻结与配对核验通过，每组56张过程图。A/B全量预测相似，形状配对后3.08%实例类别不同；无本轮官方成绩。固定4图／330实例的亮度扫描显示test101加亮0.10使A/B有5／7个类别翻转，但上部大片浅色区域未普遍改变判定，支持亮度敏感性、不证明它是主要失配原因。训练包在`output/semantic_d5a_analysis/`，诊断图在`output/brightness_probe/`；未启动新训练或修改默认推理。

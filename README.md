...
> 扩散专用工作区：`codex/diffusion-restoration-20260923`。[D1续训至3200次](docs/RGB_DIFFUSION_SHORT_DECISION_20260923.md)已通过固定训练配对工程门槛；用户随后授权的全1000图、60 epoch长测于2026-09-23 14:33（北京时间）启动，尚未确认完成。使用`rgb_restoration_diffusion_d1_all60_monitored.yaml`，从零训练、同时保存首步和16步过程输出，按用户要求不持续监控。短测不代表复赛收益。

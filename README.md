...
> 2026-09-24：[后端适配配对分析](docs/BACKEND_D4_ANALYSIS_20260924.md)已完成：两组各60轮／3840更新、0失败，同增强／初始权重／抽样；两包100图推理及双端校验通过，过程图每组56张。D4末10轮三项训练损失较无D4低6.58%／5.15%／8.64%，但微调均增加实例，强模糊补边未有明显突破；建议两包成对黑盒，不凭loss晋级。私有短目录`output/d4_adapt/`。原D4首步复赛0.8490/0.8532，旧v4仍为已验证较高总分参照。

...
> 2026-09-24：[D5a端点模糊实验](docs/RGB_DIFFUSION_D5A_20260924.md)通过GPU短测并已启动串行队列。按用户最新要求，候选与原空间配方对照都从随机初始化训练，全1000图、60轮/15000更新，不继承D4权重；唯一增强差异为约20%样本使用强弱端点平滑场。入口`tools/run_rgb_d5a.py`，短目录`outputs/d5a/`，自动保存固定训练配对和真实图缩略图。

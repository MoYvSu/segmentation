...
> “冻结主线 + 双头信息交流”对照已完成，跨头有小幅相对收益，尚未超过原主线几何；直接掩码路线停止继续投入。实验关系与入口见
...
- **双头信息交流对照已完成**：self/cross各20轮，损失均选中e20；五图匹配690/700，仍低于
  原主线715。覆盖率保持约98.75%，面积项改善但几何下降。仅语义修正恢复715匹配、面积误差
  增大，均不晋级。见[最终结果与拆分诊断](docs/MAINLINE_CROSS_HEAD_RESULTS_20260911.md)。
- **GT affinity 完整部署检验未通过**：固定正常E10a，从新GT直接导出正确连接后，mean/high=.65
  五图匹配715→1；仅把理想输入的融合改成top2恢复724，证明目标与读出失配。此为GT辅助诊断，
  不据此替换主线或宣布真实模型top2有效。见[检验与定位](docs/AFFINITY_GT_DEPLOYMENT_ORACLE_20260911.md)。
  [既有真实模型六图对照](docs/AFFINITY_CLEAN60_DIAGNOSIS_20260910.md)中，G4b/e110 的 mean→top2
  匹配仅854→856/792→806，且两者有效匹配mIoU均下降；理想输入结果不能替代该对照。
- **掩码路线已停止继续投入**：两组30轮续训已于2026-09-11完成，同预算对照e143与EMA一致性e144均已
  完成五图原尺寸评估。EMA使匹配609→615、漏检计零IoU .6398→.6479、未覆盖10.29%→8.91%，
  有小幅收益；主线同图匹配715、漏检计零IoU .7670，仍不替换部署主线。
  使用240张去除人工别名的固定伪标签，EMA实际覆盖912张在线图；已见图889继续排除。
  见[结果与下一步判断](docs/MASK_SET_CONSISTENCY_RESULTS_20260911.md)及
  [实验说明](docs/MASK_SET_CONSISTENCY_EXPERIMENT_20260911.md)。服务器已清理约25.1GiB历史临时产物。

...
> 扩散专用工作区：`codex/diffusion-restoration-20260923`。[D1全量60轮分析](docs/RGB_DIFFUSION_ALL60_ANALYSIS_20260923.md)：1000图/15000更新完整完成、零失败，后期趋于平台；16步的梯度误差高于输入且真实图颗粒增加，暂不晋级。首步的训练配对误差较低，尚无D1黑盒分数。v6本轮用户回分0.8448/0.8506、总分84.770，低于v4的85.715；保留v4作为已验证修复基准。[D2低噪声20轮分析](docs/RGB_DIFFUSION_D2_ANALYSIS_20260923.md)：5000更新零失败、48张过程图完整，训练误差较低但修复输出近乎原图；不晋级、不自动续训。[D3起点强化](docs/RGB_DIFFUSION_D3_TERMINAL50_20260923.md)已启动：保留kappa0.03，仅将t=16训练比例提高到50%，先执行原60轮日程前20轮，自动保存过程图。下游适应问题保留为独立假设。

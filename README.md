...
> 2026-09-24：[D5b短链监督](docs/RGB_DIFFUSION_D5B_20260924.md)通过59项CPU检查及GPU配对短测，16:24:26已启动正式后台队列。保持D5a网络和空间增强，从零、全1000图、60轮，在25%批次上额外监督自身后验状态的至多两步；正式对照复用已完成D5a，不载入其权重。入口`tools/run_rgb_d5b.py`，短目录`outputs/d5b/`，自动保存第1/2/3/4/8/16次输出缩略图。

> 2026-09-24：[D5a完成分析](docs/RGB_DIFFUSION_D5A_ANALYSIS_20260924.md)：候选与原空间配方对照都从零完成全1000图、60轮／15000更新、零失败，不继承D4；四类过程图与配对检查完整。强弱共存覆盖0.935%→20.392%，32张训练源的新端点退化强区RGB／梯度误差比对照降低7.06%／1.92%；真实补边未获证明，16步仍差于首步。沿用原分割后端的两份首步黑盒包已完成全量推理、双端校验与分割可视化，本地`output/d5a_analysis/`；尚无官方回分，不替换旧v4。训练入口`tools/run_rgb_d5a.py`，短目录`outputs/d5a/`。
...
> 扩散专用工作区：`codex/diffusion-restoration-20260923`。[D4与D3同预算分析](docs/RGB_DIFFUSION_D4_ANALYSIS_20260923.md)：两组均完成60轮／15000累计更新，零失败。32张训练源图的新空间模糊配对上，D4首步RGB／梯度误差比D3降低19.49%／5.38%，均匀模糊小幅退步；16步仍差于首步，真实图补边未获证明。后续D5a已完成，当前独立推进D5b短链监督。短入口为`outputs/rgb_restoration_diffusion_d4/`与`outputs/rgb_restoration_diffusion_d3/`。v4继续作为已验证修复基准，上述后端适配回分未证明不适配是主要瓶颈。

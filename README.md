...
> 2026-09-25：[D5a语义两方案](docs/SEMANTIC_D5A_20260925.md)：A保留完整语义头继续适配，B按用户选择随机初始化FPN/分类层并移除直接RGB修正。共同冻结D5a、SAM2/LoRA和affinity，全32张新GT、同端点模糊与噪声、各60轮，不留代理。结构、初始化和学习率不同，按实际方案比较。入口`tools/run_semantic_d5a.py`，短目录`outputs/semantic_d5a/`，保留固定过程缩略图，结束后自动全量推理、打包和对比渲染；官方成绩待回报。
...
> 2026-09-25：[分类来源交叉](docs/SEMANTIC_CROSS_20260925.md)完成全100图：固定已有baseline／D5a最终实例PNG，只交换原图／D5a输入的分类概率，两个原对角逐ID精确复现。D5a划分换回原图分类有6.60%实例翻转，证明最终分类会受输入变化影响，尚不能判断准确率方向。两份交叉黑盒包已校验、渲染，本地`output/semantic_cross/`，无平台提交或回分；优先测试D5a划分＋原图分类。
...
> 扩散专用工作区：`codex/diffusion-restoration-20260923`。[D4与D3同预算分析](docs/RGB_DIFFUSION_D4_ANALYSIS_20260923.md)：两组均完成60轮／15000累计更新，零失败。32张训练源图的新空间模糊配对上，D4首步RGB／梯度误差比D3降低19.49%／5.38%，均匀模糊小幅退步；16步仍差于首步，真实图补边未获证明。后续D5a与D5b均已完成。短入口为`outputs/rgb_restoration_diffusion_d4/`与`outputs/rgb_restoration_diffusion_d3/`。v4继续作为已验证修复基准，上述后端适配回分未证明不适配是主要瓶颈。

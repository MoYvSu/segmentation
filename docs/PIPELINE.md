...
2026-09-24：[D5a端点模糊](RGB_DIFFUSION_D5A_20260924.md)已通过GPU短测并启动串行训练。
用户明确不继承D4权重：候选/原空间配方对照均从零、全1000图、60轮；独立推进修复器，
后端适配两包仍待黑盒。入口`tools/run_rgb_d5a.py`，不包含短链监督，过程图自动保留。
...
D4训练loss稳定较低但不能证明真实补边，两组后端先等待黑盒；用户随后批准上方独立D5a修复实验。

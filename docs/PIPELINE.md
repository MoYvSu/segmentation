...
2026-09-24：[D5b短链监督](RGB_DIFFUSION_D5B_20260924.md)已通过59项CPU检查及GPU配对短测，
北京时间16:24:26已启动正式后台队列，代码`4834157`，仅确认启动、不持续人工监控。
保持D5a网络和增强，从零训练全1000图、60轮；25%批次额外监督自身后验状态上的两步，
跨步切梯度、辅助权重0.25，不载入D4/D5a权重。正式参照已完成的D5a，完整记录额外计算量。
入口`tools/run_rgb_d5b.py`，输出`outputs/d5b/`；固定过程图增加第3次输出，训练完成自动检查配对。

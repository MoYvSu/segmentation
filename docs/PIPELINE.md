...
初始化、结构、学习率同时不同，不作为纯结构消融；尚无本轮官方成绩。
...
私有本地`output/semantic_cross/`；优先测试`submission_grestored_sraw.zip`，
反向对照`submission_graw_srestored.zip`。两包已校验和渲染，官方成绩待用户提交后回报。
已有6.60%／6.05%实例翻转的输入分类影响证据，尚不能认定哪种类别更正确；不是纯几何／语义头完全分离。

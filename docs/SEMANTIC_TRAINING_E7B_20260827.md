...
|---|---:|---:|---:|---:|---:|
| 当前 G4b/V6 hard 基线 | 0.65 | 79.1800 | 0.83684 | 0.25324 | 1302 |
| E7b-A 整体替换 | 0.65 | 78.8379 | 0.83655 | 0.25979 | 1302 |

历史 `88.0551` 使用的是较早的 G4b 训练配置后处理，未包含当前完全一致的
seal2/reconstruct 部署口径，不能与 E7b 的 `78.8379` 直接比较。严格同口径下 E7b 仍略有
退化，但总分只下降 `0.3421`，而不是约 9 分；这与目检中“总体接近、每图修正少量实例”
一致。

因此 E7b-A 暂不晋级为默认语义 checkpoint，也不应立即在其上开放 LoRA。68 张测试图中，
...
当前回退点仍为 G4b/V6；E7b 保留为有局部纠错价值的 challenger。下一步先把 V6 几何、
V6 默认投票与 E7b 候选分类解耦，通过保守门控保留少量修正，再决定是否训练残差语义头。

## 后续 E7b-B

只有保守 challenger 或后续残差训练出现明确正信号后，才考虑开放后层 LoRA。LoRA 一旦
变化，必须重新训练/校准 affinity geometry，不能直接复用 G4b。

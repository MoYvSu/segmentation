...
**最新诊断已完成：** GT直接导出affinity，正常E10a及完整部署链固定，五图匹配715→1；
仅改理想输入的短程融合mean→top2恢复724，证实目标到读出失配。真实模型与部署配置保持原样。
见[GT affinity完整部署检验](AFFINITY_GT_DEPLOYMENT_ORACLE_20260911.md)。
真实G4b/e110的六图mean→top2已测为854→856/792→806匹配，且有效匹配mIoU均下降；
不是待重跑的新实验，见[真实模型对照](AFFINITY_CLEAN60_DIAGNOSIS_20260910.md)。

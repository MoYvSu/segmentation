...
2026-09-11 更新：**两组均已完成 120 轮，整个作业于 02:00 正常结束；损失最优 e112/e120 的完整原尺寸比较已完成。** 伪标签训练方案改善几何，归属约束减少碎片但增加空缺，仍不替换 E10a+G4b。详见[结果与一致性学习决策](MASK_SET_TEACHER_RESULTS_20260911.md)。下文保留实验设计和当时启动快照，后续状态以下方更正及新实验记录为准。

后续更正：跨目录内容检查发现原249张中有9张人工图别名，其中train_589对应验证图train_889。
原生成器仅核对无标签目录内的同名排除内容，未读取人工目录真实内容，下文当时的排除设计没有
完全实现。原数据归档保留，新的读取检查会拒绝其别名冲突。已建立240张不重复制图的子集，
并从e120启动[五图选优的一致性对照](MASK_SET_CONSISTENCY_EXPERIMENT_20260911.md)；889保留为已见图诊断。
...
- 未修改模型架构或最终推理；新增损失的正式最终输出比较已另记于[结果报告](MASK_SET_TEACHER_RESULTS_20260911.md)。
...
上述命令为当时后台启动记录，本次作业现已完成，勿当作待执行任务重复运行。此前前台生成被切换为可恢复的后台批处理，已生成的样本按原清单复用，未改标签。作业顺序：
...
收到用户训练结束通知后，已核对两组完成标记、best_joint epoch、实际样本计数与更新，并使用 `tools/evaluate_mask_set.py` 完成同六图、新 GT、原尺寸比较，取回固定局部及几何诊断。结果见[2026-09-11 报告](MASK_SET_TEACHER_RESULTS_20260911.md)，原始训练快照保留当时的 `final_evaluation=pending`。
